...
module.exports = {
    ...
    async execute(msg, args) {
        ...
        if (guild.description == 0) {
            emb.setDescription("У данного сервера не установлено описание")
            return msg.channel.send(emb.setColor(colors.error))
        }

        const gChannel = await msg.guild.channels.cache.get(guild.channel)
        if (guild.channel == 0 || !gChannel) return msg.channel.send(emb.setDescription('Укажите канал для bump').setColor(colors.error))
        ...
        } catch { return msg.channel.send(emb.setDescription("**Не удалось создать ссылку для bump**").setColor(colors.error)) }
